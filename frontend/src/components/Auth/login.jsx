...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    try {
      await axios.post('http://localhost:4000/api/v1/user/login', { email, password, role, age }, { withCredentials: true, headers: { 'Content-Type': 'application/json' } });
      toast.success('User Login Successful');
      setEmail("");
      setPassword("");
      setRole("");
      setAge(""); // Reset age after successful login
      setIsAuthorized(true);
      navigateTo('/');
    } catch (error) {
